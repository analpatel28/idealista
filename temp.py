    ...
    def get_url(self):
        ...
        while True:
            try:
                ...
                all_url = ['https://www.idealista.pt/en/imovel/32633323/']
        ...
        for url in all_url:
            ...
            try:
                ...
                if not response.json():
                    try:
                        p_req = requests.get(f'http://api.scrape.do?token=c70858af0cc14fe18d60d8ac91453c7380303d31a13&url={property_url}',headers=self.header)
                    ...
                    if p_req.status_code == 200:
                        ...
                        try:
                            price = int("".join(p_res.xpath('//strong[@class="price"]/text()')).replace("€/month", "").replace("€","").replace(",", ""))
                        except:
                            price = 0

                        try:
                            if not price:
                                price = int("".join(p_res.xpath('//div[@class="info-data"]/span[contains(text(), "Price from")]/span/text()')).replace("€", "").replace(",", "").replace("EUR ", "").replace("EUR", "").strip())
                        except:
                            price = 0
                        if not price:
                            # self.Update_sql(p_url, status='check')
                        ...
                        try:
                            t_num = ",".join(
                                p_res.xpath('//div[@class="info-features"]//span[contains(text(),"T")]/text()')).replace("T","").strip()
                        ...
                        try:
                            description = etree.tostring(p_res.xpath('//div[@class="adCommentsLanguage expandable is-expandable"]//p')[0]).decode("utf-8")
                        except:
                            description = "."
                        ...
                        try:
                            size_built = float(p_res.xpath('//div[@class="info-features"]/span/span/text()')[0].replace(",","").strip())
                        except:
                            size_built = 0
                        ...
                        try:
                            energy_rating = "".join(p_res.xpath('//*[contains(text(),"Energy efficiency rating: ")]/span/@title'))
                        except:
                            energy_rating = ""

                        Energy_Rating = post_api.Energy_Rating(energy_rating)

                        pool = p_res.xpath('//div[@class="details-property_features"]/ul/li[contains(text(),"pool")]/text()')
                        if pool:
                            pool = True
                        else:
                            pool = False
